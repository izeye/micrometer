...
import io.micrometer.test.support.testcontainers.SkippableContainer;
...
abstract class AbstractElasticsearchMeterRegistryIntegrationTest {
    ...
    @Container
    private final SkippableContainer<ElasticsearchContainer> elasticsearch = new SkippableContainer<>(
            () -> new ElasticsearchContainer(getDockerImageName(getVersion())));
    ...
    @BeforeEach
    void setUp() {
        host = "http://" + elasticsearch.getContainer().getHttpHostAddress();
